...
public struct SwiftAsyncShim {
    ...
    public struct Method: Sendable {
        ...
        /// A struct result that crosses as a record (P1.3c).
        public var structResult: RecordStruct?
        /// A boxed struct result, by its Swift name (P1.3e).
        public var opaqueResult: String?
        /// Boxed struct parameters, by index, with their Swift names (P1.3e).
        public var opaqueParameters: [Int: String] = [:]
    ...
    /// A struct that crosses as a BASIC TYPE record (P1.3c): one slot per
    /// leaf, in the order its initializer takes them.
    public struct RecordStruct: Sendable, Equatable {
        public let swiftName: String
        /// The Swift path to each leaf from a value: `origin.x`.
        public let leafPaths: [String]
        /// Each leaf's Swift type: `Double`, `CGFloat`, `Int`, `Bool` or `String`.
        public let leafTypes: [String]
        /// The struct rebuilt from `v0`, `v1`, ...
        public let build: String
        public var stem: String { swiftName.replacingOccurrences(of: ".", with: "_") }
        public init(swiftName: String, leafPaths: [String], leafTypes: [String], build: String) {
            self.swiftName = swiftName
            self.leafPaths = leafPaths
            self.leafTypes = leafTypes
            self.build = build
        }
    }

    /// A property whose immutable struct crosses in a box (P1.3e).
    public struct OpaqueProperty: Sendable {
        public let className: String
        public let name: String
        /// The struct's Swift name, which the box's value is cast back to.
        public let swiftName: String
        /// Whether it may be nil, which crosses as NULL.
        public let isOptional: Bool
        public let isSettable: Bool
        public init(className: String, name: String, swiftName: String, isOptional: Bool, isSettable: Bool) {
            self.className = className
            self.name = name
            self.swiftName = swiftName
    ...
    /// A property whose struct crosses as a record (P1.3c).
    public struct StructProperty: Sendable {
        public let className: String
        public let name: String
        public let type: RecordStruct
        public let isSettable: Bool
        public init(className: String, name: String, type: RecordStruct, isSettable: Bool) {
            self.className = className
            self.name = name
            self.type = type
            self.isSettable = isSettable
    ...
    /// Its closure properties BASIC can assign a handler to (P1.2).
    public let handlerProperties: [HandlerProperty]
    /// Its properties whose struct crosses as a record (P1.3c).
    public let structProperties: [StructProperty]
    /// Its properties whose struct crosses in a box (P1.3e).
    public let opaqueProperties: [OpaqueProperty]

    /// A closure property BASIC assigns: `Slider.onChange = FUNCTION(V AS DOUBLE) AS DOUBLE = ...`.
    public struct HandlerProperty: Sendable {
        public let className: String
        public let name: String
        /// Each parameter's Swift type: `Int`, `Double` or `Bool`.
        public let parameters: [String]
        /// The result's Swift type, or nil for `Void`.
        public let returns: String?
        /// Whether the property is optional, so assigning NULL clears it.
        public let isOptional: Bool
        public init(className: String, name: String, parameters: [String], returns: String?, isOptional: Bool) {
            self.className = className
            self.name = name
    ...
    public init(module: String, methods: [Method], properties: [EnumProperty] = [], payloadProperties: [PayloadProperty] = [],
                handlerProperties: [HandlerProperty] = [], structProperties: [StructProperty] = [],
                opaqueProperties: [OpaqueProperty] = []) {
        self.handlerProperties = handlerProperties
        self.structProperties = structProperties
        self.opaqueProperties = opaqueProperties
        self.module = module
    ...
    public func source() -> String? {
        guard !methods.isEmpty || !properties.isEmpty || !payloadProperties.isEmpty || !handlerProperties.isEmpty
              || !structProperties.isEmpty || !opaqueProperties.isEmpty else { return nil }
        var lines = [
            ...
            "",
            "/// Boxes for immutable structs BASIC holds by reference (P1.3e).",
            "@_silgen_name(\"basic_rt_swift_opaque_make\")",
            "private func basicOpaqueMake(_ value: Any, _ typeIndex: Int) -> UnsafeMutableRawPointer",
            "@_silgen_name(\"basic_rt_swift_opaque_value\")",
            "private func basicOpaqueValue(_ p: UnsafeMutableRawPointer?) -> Any",
            "",
        ...
        for method in methods {
            ...
            for (index, type) in method.parameterTypes.enumerated() {
                ...
                } else if method.opaqueParameters[index] != nil {
                    parameters.append("_ a\(index): UnsafeMutableRawPointer?")
                    callArguments.append("b\(index)")
            ...
            if method.payloadResult != nil || method.structResult != nil || method.opaqueResult != nil {
                parameters.append("_ ti: Int")
            }
            let result: String
            if let boxed = method.opaqueResult, boxed.hasSuffix("?") {
                // An optional box: nil crosses as NULL (P1.3e).
                result = " -> UnsafeMutableRawPointer?"
            } else if method.isInitializer || method.objectResult != nil || method.arrayResult != nil || method.stringResult
                || method.payloadResult != nil || method.structResult != nil || method.opaqueResult != nil {
                result = " -> UnsafeMutableRawPointer"
            ...
            func handOut(_ expression: String) -> String {
                ...
                if let type = method.structResult {
                    return "basicStructOut_\(type.stem)(\(expression), ti)"
                }
                if let boxed = method.opaqueResult {
                    return boxed.hasSuffix("?") ? "(\(expression)).map { basicOpaqueMake($0, ti) }" : "basicOpaqueMake(\(expression), ti)"
                }
            ...
            // A box's value, cast back to the struct it holds (P1.3e).
            for (index, swiftName) in method.opaqueParameters.sorted(by: { $0.key < $1.key }) {
                if swiftName.hasSuffix("?") {
                    // NULL is nil.
                    lines.append("    let b\(index) = a\(index).map { basicOpaqueValue($0) as! \(swiftName.dropLast()) }")
                } else {
                    lines.append("    let b\(index) = basicOpaqueValue(a\(index)) as! \(swiftName)")
                }
            }
            ...
            if method.isAsync {
                ...
                } else if let boxed = method.opaqueResult {
                    // A box, made inside the isolated closure and carried out as
                    // a bit pattern, as a record is. Nil is the zero pattern,
                    // which comes back out as NULL.
                    if boxed.hasSuffix("?") {
                        lines.append("    let bits = MainActor.assumeIsolated { (\(called)).map { Int(bitPattern: basicOpaqueMake($0, ti)) } ?? 0 }")
                        lines.append("    return UnsafeMutableRawPointer(bitPattern: bits)")
                    } else {
                        lines.append("    let bits = MainActor.assumeIsolated { Int(bitPattern: basicOpaqueMake(\(called), ti)) }")
                        lines.append("    return UnsafeMutableRawPointer(bitPattern: bits)!")
                    }
                } else if let type = method.structResult {
                    // A record, built inside the isolated closure and carried out
                    // as a bit pattern, as a payload enum's is.
                    lines.append("    let bits = MainActor.assumeIsolated { Int(bitPattern: basicStructOut_\(type.stem)(\(called), ti)) }")
                    lines.append("    return UnsafeMutableRawPointer(bitPattern: bits)!")
        ...
        // Handler properties (P1.2). The thunk hands over the BASIC closure's
        // compiled body and its environment; the body is called as the C
        // function it is — `(environment, arguments...) -> result`, numbers as
        // doubles and booleans as `Bool` — so nothing generic runs per event.
        if !handlerProperties.isEmpty {
            lines += [
                "/// A BASIC closure's environment, carried into a Swift closure. The",
                "/// closure is retained by the thunk and its environment never changes.",
                "private struct BASICEnvironment: @unchecked Sendable {",
        ...
        for property in handlerProperties {
            ...
            lines.append("    guard let fn else {")
            if property.isOptional {
                lines.append("        MainActor.assumeIsolated { object.`\(property.name)` = nil }")
            }
            lines.append("        return")
            lines.append("    }")
        ...
        // Structs as records (P1.3c): a value out into a new record, one slot
        // per leaf; a record back into the struct its initializer builds.
        var recordStructs: [RecordStruct] = []
        for method in methods { if let type = method.structResult, !recordStructs.contains(type) { recordStructs.append(type) } }
        for property in structProperties where !recordStructs.contains(property.type) { recordStructs.append(property.type) }
        for type in recordStructs.sorted(by: { $0.stem < $1.stem }) {
            lines.append("private func basicStructOut_\(type.stem)(_ value: \(type.swiftName), _ ti: Int) -> UnsafeMutableRawPointer {")
            // A record made with a zero first slot, which the loop below
            // writes along with every other.
            lines.append("    let record = basicEnumMake(ti, 0)")
            for (slot, (path, swift)) in zip(type.leafPaths, type.leafTypes).enumerated() {
                switch swift {
                case "Bool": lines.append("    basicEnumSetBoolean(record, \(slot), value.\(path))")
                case "String": lines.append("    basicEnumSetString(record, \(slot), value.\(path))")
                default: lines.append("    basicEnumSetNumber(record, \(slot), Double(value.\(path)))")
                }
            }
            lines.append("    return record")
            lines.append("}")
            lines.append("private func basicStructIn_\(type.stem)(_ record: UnsafeMutableRawPointer?) -> \(type.swiftName) {")
            for (slot, swift) in type.leafTypes.enumerated() {
                switch swift {
                case "Bool": lines.append("    let v\(slot) = basicEnumBoolean(record, \(slot))")
                case "String": lines.append("    let v\(slot) = basicEnumString(record, \(slot))")
                case "Double": lines.append("    let v\(slot) = basicEnumNumber(record, \(slot))")
                default: lines.append("    let v\(slot) = \(swift)(basicEnumNumber(record, \(slot)))")
                }
            }
            lines.append("    return \(type.build)")
            lines.append("}")
            lines.append("")
        }
        for property in opaqueProperties {
            lines.append("@_cdecl(\"\(property.getter)\")")
            if property.isOptional {
                lines.append("public func \(property.getter)(_ me: UnsafeMutableRawPointer, _ ti: Int) -> UnsafeMutableRawPointer? {")
                lines.append("    let object = Unmanaged<\(property.className)>.fromOpaque(me).takeUnretainedValue()")
                lines.append("    let bits = MainActor.assumeIsolated { object.`\(property.name)`.map { Int(bitPattern: basicOpaqueMake($0, ti)) } ?? 0 }")
                lines.append("    return UnsafeMutableRawPointer(bitPattern: bits)")
            } else {
                lines.append("public func \(property.getter)(_ me: UnsafeMutableRawPointer, _ ti: Int) -> UnsafeMutableRawPointer {")
                lines.append("    let object = Unmanaged<\(property.className)>.fromOpaque(me).takeUnretainedValue()")
                lines.append("    let bits = MainActor.assumeIsolated { Int(bitPattern: basicOpaqueMake(object.`\(property.name)`, ti)) }")
                lines.append("    return UnsafeMutableRawPointer(bitPattern: bits)!")
            }
            lines.append("}")
            lines.append("")
            guard property.isSettable else { continue }
            lines.append("@_cdecl(\"\(property.setter)\")")
            lines.append("public func \(property.setter)(_ me: UnsafeMutableRawPointer, _ value: UnsafeMutableRawPointer?) {")
            lines.append("    let object = Unmanaged<\(property.className)>.fromOpaque(me).takeUnretainedValue()")
            if property.isOptional {
                lines.append("    let converted = value.map { basicOpaqueValue($0) as! \(property.swiftName) }")
            } else {
                lines.append("    let converted = basicOpaqueValue(value) as! \(property.swiftName)")
            }
            lines.append("    MainActor.assumeIsolated { object.`\(property.name)` = converted }")
            lines.append("}")
            lines.append("")
        ...
        for property in structProperties {
            ...
            lines.append("public func \(property.getter)(_ me: UnsafeMutableRawPointer, _ ti: Int) -> UnsafeMutableRawPointer {")
            lines.append("    let object = Unmanaged<\(property.className)>.fromOpaque(me).takeUnretainedValue()")
            lines.append("    let bits = MainActor.assumeIsolated { Int(bitPattern: basicStructOut_\(property.type.stem)(object.`\(property.name)`, ti)) }")
            lines.append("    return UnsafeMutableRawPointer(bitPattern: bits)!")
            lines.append("}")
            lines.append("")
            guard property.isSettable else { continue }
            ...
            lines.append("public func \(property.setter)(_ me: UnsafeMutableRawPointer, _ value: UnsafeMutableRawPointer?) {")
            lines.append("    let object = Unmanaged<\(property.className)>.fromOpaque(me).takeUnretainedValue()")
            lines.append("    let converted = basicStructIn_\(property.type.stem)(value)")
            lines.append("    MainActor.assumeIsolated { object.`\(property.name)` = converted }")
            lines.append("}")
            lines.append("")
        }
