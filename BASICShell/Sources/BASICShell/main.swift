...
/// `COMPILE [file.bas] [-o output]` — hands the program to `basicc`, the
/// native compiler that lives beside this shell (BASIC_COMPILER.md, Phase
/// 6.5). With no file, the program in memory is saved to a temporary file
/// and compiled; the output defaults to the source's base name in the
/// current directory. Diagnostics come back in basicc's `file:line: error:`
/// form, the same text the IDEs parse.
///
/// `basicc` is found through `$BASICC`, then the login shell's PATH, then
/// the sibling package's own build products — so a checkout works before
/// anything is installed.
@MainActor
func handleCompileCommand(_ line: String) -> Bool {
    let trimmed = line.trimmingCharacters(in: .whitespacesAndNewlines)
    let words = shellWords(in: trimmed)
    guard words.first?.uppercased() == "COMPILE" else { return false }
    ...
    while index < words.count {
        ...
        if word == "-o" {
            ...
            guard index < words.count else {
                host.printLine("COMPILE: -o needs a path")
            ...
            host.printLine("COMPILE: usage: COMPILE [file.bas] [-o output]")
            session.lastSystemStatus = 2
            return true
        }
        index += 1
    ...
    guard let basicc = locateBASICC() else {
        host.printLine("COMPILE: basicc not found — build Code/BASICCompiler (swift build) or set BASICC=/path/to/basicc")
    ...
    if source == nil {
        guard !session.program.isEmpty else {
            host.printLine("COMPILE: nothing to compile — LOAD a program or name a file")
            session.lastSystemStatus = 1
            return true
        }
        let path = NSTemporaryDirectory() + "basicshell-compile-\(ProcessInfo.processInfo.processIdentifier).bas"
        do {
            try session.program.listing().write(toFile: path, atomically: true, encoding: .utf8)
        } catch {
            host.printLine("COMPILE: could not save the program: \(error.localizedDescription)")
            session.lastSystemStatus = 1
            return true
        }
        source = path
    ...
    let output = outputPath ?? ((source! as NSString).lastPathComponent as NSString).deletingPathExtension
    let process = Process()
    process.executableURL = URL(fileURLWithPath: basicc)
    process.arguments = ["build", source!, "-o", output]
    process.currentDirectoryURL = URL(fileURLWithPath: (try? host.currentDirectoryPath()) ?? FileManager.default.currentDirectoryPath)
    let pipe = Pipe()
    ...
    do {
        ...
        host.printLine("COMPILE: could not run basicc: \(error.localizedDescription)")
        session.lastSystemStatus = 1
        return true
    }
    let transcript = String(decoding: pipe.fileHandleForReading.readDataToEndOfFile(), as: UTF8.self)
    process.waitUntilExit()
    for line in transcript.split(separator: "\n", omittingEmptySubsequences: true) {
        host.printLine(String(line))
    }
    if process.terminationStatus == 0 {
        host.printLine("compiled \(output)")
    }
    session.lastSystemStatus = Int(process.terminationStatus)
    return true
}

/// The compiler beside this shell: `$BASICC`, the login PATH, or the sibling
/// package's build products.
@MainActor
func locateBASICC() -> String? {
    let fileManager = FileManager.default
    if let override = ProcessInfo.processInfo.environment["BASICC"], fileManager.isExecutableFile(atPath: override) {
        return override
    }
    let lookup = Process()
    lookup.executableURL = URL(fileURLWithPath: "/bin/zsh")
    lookup.arguments = ["-lc", "command -v basicc"]
    let pipe = Pipe()
    ...
    if (try? lookup.run()) != nil {
        ...
        if !found.isEmpty, fileManager.isExecutableFile(atPath: found) { return found }
    }
    // Code/BASICShell/.build/<config>/BASICShell → Code/BASICCompiler/.build/<config>/basicc
    let executable = URL(fileURLWithPath: Bundle.main.executablePath ?? CommandLine.arguments[0]).standardizedFileURL
    let code = executable.deletingLastPathComponent().deletingLastPathComponent().deletingLastPathComponent().deletingLastPathComponent()
    for configuration in ["debug", "release"] {
        let candidate = code.appendingPathComponent("BASICCompiler/.build/\(configuration)/basicc").path
